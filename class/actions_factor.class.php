...
class ActionsFactor
{
	...
	{
		if (in_array('invoicecard', explode(':', $parameters['context'])) && $action == 'create') {
			
			$this->selectBankFromSoc(GETPOST('socid'),'#selectfk_account');
			
		}
		
	}

	private function selectBankFromSoc($fk_soc, $input_bank='#fk_bank') {
                ...
                if(!empty($societe->array_options['options_fk_soc_factor']) && $societe->array_options['options_factor_suivi'] == 1) {
                        ...
                        /* selectbankaccount */
                                $(document).ready(function() {
                                        $('<?php echo $input_bank; ?>').val('<?php echo $factor->fk_bank_account; ?>');
                                });
	...
	function formBuilddocOptions($parameters, &$object, &$action, $hookmanager)
		...
		if (in_array('formfile', explode(':', $parameters['context'])) && $parameters['modulepart'] == 'facture')
                {
                        //ob_start();
                        $this->selectBankFromSoc($object->socid);
